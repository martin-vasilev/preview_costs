checkFlags<- function(maxtrial=80, list_asc= "Experiment 2/preproc/files_fxd.txt"){
  ...
  trial_info<- function(file, maxtrial){ # extracts information for processing trials
    ...
    if(length(duplicated)>0){ # if there were aborted trials..
      ...
      toBeRemoved<- unique(toBeRemoved)
      ...
      #if(length(start)!=length(ID) & length(end)== length(start)){
      #  start<- start[-toBeRemoved]
      #  end<- end[-toBeRemoved]
      #}
  ...
  wrds<- read_delim("Experiment 2/preproc/nwords.txt", 
  ...
  for(i in 1:length(asc)){ # for each subject
    ...
    trial_db<- suppressMessages(trial_info(dataF, maxtrial)) # get info about trials)
